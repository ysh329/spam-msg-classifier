...
# !/usr/bin/python
################################### PART0 DESCRIPTION #################################
# Filename: class_read_test_text_to_database.py
# Description:
#
...
class ReadTestText2DB(object):

    def __init__(self, database_name, test_data_dir, stopword_data_dir, pyspark_sc):
        ...
        # read test text
        try:
            self.test_f = open(test_data_dir, 'r')
            logging.info("Success in reading %s." % test_data_dir)
        except Exception as e:
            logging.error("Fail in reading %s." % test_data_dir)
    ...
    def __del__(self):
        try:
            self.test_f.close()
            logging.info("Success in closing test data file.")
        except Exception as e:
            logging.error("Fail in closing test data file.")
            logging.error(e)
        ...
        logging.info("END CLASS {class_name}.".format(class_name = ReadTestText2DB.__name__))
        self.end = time.clock()
        logging.info("The class {class_name} run time is : {delta_time} seconds".format(class_name = ReadTestText2DB.__name__, delta_time = self.end))



    def read_test_data(self, test_data_dir, stopword_data_dir, is_train=0, true_label='-1'):
        # sub function
        ...
        test_data_rdd = self.sc.textFile(name = test_data_dir, minPartitions = 1, use_unicode = True)
        test_data_triple_rdd = test_data_rdd.map(lambda line: line.split("\t"))\
            .map(lambda (id, content): (id.strip(),\
                                                    true_label.strip(),\
                                                    content.strip().replace("'", '"').replace("\\", "")\
                                                   )\
                 )
        #test_data_rdd.persist(storageLevel = StorageLevel.MEMORY_ONLY_SER)
        ...
        is_train = 0
        cleaned_and_processed_test_data_rdd = test_data_triple_rdd.map(lambda (id,\
                                                                                true_label,\
                                                                                content): (id,\
                                                                                           is_train,\
                                                                                           true_label,\
        ...
        logging.info("cleaned_and_processed_test_data_rdd.count():{0}".format(cleaned_and_processed_test_data_rdd.count()))
        logging.info("cleaned_and_processed_test_data_rdd.take(1):{0}".format(cleaned_and_processed_test_data_rdd.take(1)))
        logging.info("len(cleaned_and_processed_test_data_rdd.take(1)):{0}".format(len(cleaned_and_processed_test_data_rdd.take(1))))
        return cleaned_and_processed_test_data_rdd



    def message_insert_sql_generator(self, database_name, message_table_name, cleaned_and_processed_test_data_rdd):
        message_insert_sql_rdd = cleaned_and_processed_test_data_rdd.map(lambda (id,\
                                                                                  is_train,\
                                                                                  true_label,\
                                                                                  content,\
                                                                                  word_num,\
    ...
    def save_test_data_to_database(self, message_insert_sql_rdd):
...
test_data_dir = "../data/input/test_set_20W.txt"
...
Reader = ReadTestText2DB(database_name = database_name,
...
cleaned_and_processed_test_data_rdd = Reader.read_test_data(test_data_dir = test_data_dir,
                                                              stopword_data_dir = stopword_data_dir)
message_insert_sql_rdd = Reader.message_insert_sql_generator(database_name = database_name,
                                                             message_table_name = message_table_name,
                                                             cleaned_and_processed_test_data_rdd = cleaned_and_processed_test_data_rdd)
Reader.save_test_data_to_database(message_insert_sql_rdd = message_insert_sql_rdd)
